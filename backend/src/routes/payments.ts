...
import { validatePaymentData } from '../validators/payment';
import MessProfile from '../models/MessProfile';

const router: Router = Router();

// POST /api/payments/process - Process a payment
router.post('/process', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
        const { messId, amount, method, plan, notes } = req.body;

    // Validate payment data
    const validation = validatePaymentData({ messId, amount, method, plan, notes });
...
router.put('/:membershipId/status', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
    ...
    const success = await PaymentService.updatePaymentStatus(membershipId, status, notes);
...
router.get('/history', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
  ...
  try {
    ...
    const history = await PaymentService.getPaymentHistory(userId, messId as string);
...
router.get('/membership/:membershipId', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
        const { membershipId } = req.params;
    ...
    if (membership.userId.toString() !== userId && (req as any).user.role !== 'admin') {
      return res.status(403).json({
        success: false,
...
router.post('/remind', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
...
router.get('/overdue', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
        const userRole = (req as any).user.role;
    ...
    let query: any = { paymentStatus: 'overdue', status: 'active' };
    ...
    const overdueMemberships = await MessMembership.find(query)
...
router.post('/bulk-remind', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user.id;
  try {
        const userRole = (req as any).user.role;
