...
import { checkCanAddMeals } from '../middleware/checkSubscription';
...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    const uniqueName = `meal_${Date.now()}_${Math.random().toString(36).substring(7)}${path.extname(file.originalname)}`;
    cb(null, uniqueName);
  }
});
...
router.get('/', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const filter: any = { messId: messProfile._id.toString() };
...
router.post('/', requireAuth, checkCanAddMeals, upload.single('image'), async (req: Request, res: Response, _next: NextFunction) => {    const userId = (req as any).user.id;
    try {
        const messProfile = await MessProfile.findOne({ userId: (req as any).user.id });
    ...
    if (req.file) {
      const filePath = path.join(__dirname, '../../uploads/meals', req.file.filename);
...
router.get('/:id', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {    const userId = (req as any).user.id;
    try {
        const messProfile = await MessProfile.findOne({ userId: (req as any).user.id });
...
router.put('/:id', requireAuth, upload.single('image'), async (req: Request, res: Response, _next: NextFunction) => {    const userId = (req as any).user.id;
    try {
        const messProfile = await MessProfile.findOne({ userId: (req as any).user.id });
    ...
    try {
      ...
      if (typeof associatedMealPlans === 'string') parsedMealPlans = JSON.parse(associatedMealPlans);
      if (typeof nutritionalInfo === 'string') parsedNutritionalInfo = JSON.parse(nutritionalInfo);
      if (typeof allergens === 'string') parsedAllergens = JSON.parse(allergens);
    ...
    if (req.file) {
      ...
      if (existingMeal.imageUrl) {
        const oldImagePath = path.join(__dirname, '../..', existingMeal.imageUrl);
    ...
    if (parsedMealPlans !== undefined) updateData.associatedMealPlans = parsedMealPlans;
    if (parsedNutritionalInfo !== undefined) updateData.nutritionalInfo = parsedNutritionalInfo;
    if (parsedAllergens !== undefined) updateData.allergens = parsedAllergens;
...
router.get('/date/:date', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {    const userId = (req as any).user.id;
    try {
        const messProfile = await MessProfile.findOne({ userId: (req as any).user.id });
...
router.get('/user/today', requireAuth, async (req: Request, res: Response, _next: NextFunction) => {    const userId = (req as any).user.id;
    try {
    ...
    for (const meal of meals) {
      ...
      const hasValidMealPlan = mealAssociatedPlans.some((planId: string) => 
        messInfo.mealPlanIds.includes(planId)
      );
