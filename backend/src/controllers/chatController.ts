...
export class ChatController {
   ...
   */
  static async getUserRooms(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  static async createIndividualChat(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      ...
      if (currentUser.role === 'admin') {
        room = await ChatService.createIndividualChatForAdmin(userId, targetUserId);
      } else {
        room = await ChatService.createIndividualChat(userId, targetUserId);
      }
      ...
      logger.error('Error creating individual chat:', error);
      next(error);
    }
  }
   ...
   * Get room details
   */
  static async getRoomDetails(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      ...
      // Check permission
      const { hasPermission, room } = await ChatService.checkRoomPermission(userId, roomId, 'read');
      if (!hasPermission) {
        res.status(403).json({
          success: false,
   ...
   */
  static async updateRoomSettings(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      ...
      // Check permission
      const { hasPermission, room } = await ChatService.checkRoomPermission(userId, roomId, 'manage');
      if (!hasPermission) {
        res.status(403).json({
          success: false,
      ...
      const updatedRoom = await ChatService.updateRoomSettings(roomId, userId, {
   ...
   */
  static async getRoomMessages(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      const { roomId } = req.params;
      ...
      const messages = await ChatService.getRoomMessages(userId, roomId, page, limit);
   ...
   */
  static async markMessagesAsRead(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      const { roomId } = req.params;
      ...
      await ChatService.markMessagesAsRead(userId, roomId, messageIds);
   ...
   */
  static async getMessMembers(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      ...
      const members = memberships.map(membership => ({
        ...
        profilePicture: (membership.userId as any).profilePicture,
        role: (membership.userId as any).role,
      ...
      logger.error('Error getting mess members:', error);
      next(error);
    }
  }
   ...
   */
  static async checkCommunicationPermission(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      ...
      const canCommunicate = await ChatService.canUsersCommunicate(userId, targetUserId);
      ...
      logger.error('Error checking communication permission:', error);
      next(error);
    }
  }
   ...
   */
  static async addReaction(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      const { messageId } = req.params;
      ...
      const message = await ChatService.addReaction(userId, messageId, emoji);
   ...
   */
  static async massDeleteMessages(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = (req as any).user._id;
      const { messageIds } = req.body;
      ...
      const deletedCount = await ChatService.massDeleteMessages(userId, messageIds);
      ...
      logger.error('Error in mass delete messages:', error);
      next(error);
    }
  }
}
