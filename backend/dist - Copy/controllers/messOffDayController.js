...
exports.saveDefaultOffDaySettings = exports.getDefaultOffDaySettings = exports.getMessOffDayStats = exports.deleteMessOffDay = exports.updateMessOffDay = exports.createMessOffDay = exports.getMessOffDays = void 0;
const MessOffDay_1 = __importDefault(require("../models/MessOffDay"));
const DefaultOffDaySettings_1 = __importDefault(require("../models/DefaultOffDaySettings"));
...
const MessOffDayAudit_1 = __importDefault(require("../models/MessOffDayAudit"));
// Helper function to get mess profile for a mess owner
const getMessProfileForOwner = async (messOwnerId) => {
    return await MessProfile_1.default.findOne({ userId: messOwnerId });
};
// Get all mess off days
const getMessOffDays = async (req, res, _next) => {
    try {
        ...
        const query = { messId: messProfile._id };
        // Add date filtering
        ...
        const offDays = await MessOffDay_1.default.find(query)
            .populate('createdBy', 'firstName lastName')
            // Show most recently updated first; then by offDate desc for stability
            .sort({ updatedAt: -1, offDate: -1 })
            .skip(skip)
            .limit(Number(limit));
        const total = await MessOffDay_1.default.countDocuments(query);
        return res.json({
            success: true,
            data: {
                offDays,
...
exports.getMessOffDays = getMessOffDays;
// Create a new mess off day
const createMessOffDay = async (req, res, _next) => {
    try {
        const messOwnerId = req.user.id;
        ...
        const parseAndNormalizeDate = (dateInput) => {
            let date;
        ...
        try {
            ...
            if (!offDate && startDate && endDate) {
                ...
                for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
                    ...
                    if (copy < today)
                        return res.status(400).json({ success: false, message: 'Off day cannot be in the past' });
                ...
                if (selectedDate < today)
                    return res.status(400).json({ success: false, message: 'Off date cannot be in the past' });
        ...
        for (const ent of entries) {
            const existsActive = await MessOffDay_1.default.findOne({ messId: messProfile._id, offDate: ent.date, status: 'active' });
            if (existsActive) {
        ...
        if (hasRange) {
            ...
            const doc = new MessOffDay_1.default({
            ...
            await doc.save();
            await MessOffDayAudit_1.default.create({ messId: messProfile._id, offDayId: doc._id, action: 'create', by: messOwnerId, snapshot: doc.toObject() });
        ...
        if (subscriptionExtension) {
            const memberships = await MessMembership_1.default.find({ messId: messProfile._id, status: { $in: ['active', 'pending'] } });
            const planIds = [...new Set(memberships.map((m) => String(m.mealPlanId)).filter(Boolean))];
            const plans = await MealPlan_1.default.find({ _id: { $in: planIds } });
            ...
            for (const membership of memberships) {
                ...
                let userLeaves = [];
                try {
                    const UserLeaveModel = (await Promise.resolve().then(() => __importStar(require('../models/UserLeave')))).default;
                ...
                let missedMeals = 0;
                for (const ent of entries) {
                    const eligible = ent.meals.filter((m) => plan.mealOptions?.[m]);
        ...
        if (sendAnnouncement && entries.length > 0) {
            try {
                const room = await ChatRoom_1.default.findOne({ messId: messProfile._id, isDefault: true });
                if (room) {
                    const formatDate = (dt) => {
                    ...
                    await chatService_1.ChatService.sendMessage(messOwnerId, { roomId: room._id.toString(), content, type: 'text' });
                }
            }
            catch (e) {
...
exports.createMessOffDay = createMessOffDay;
// Update mess off day
const updateMessOffDay = async (req, res, _next) => {
    try {
        const { id } = req.params;
        ...
        const offDay = await MessOffDay_1.default.findOne({ _id: id, messId: messProfile._id });
        if (!offDay) {
        ...
        const effectiveSubscriptionExtension = (typeof subscriptionExtension === 'boolean')
            ...
            : Boolean((await MessOffDay_1.default.findById(id))?.subscriptionExtension);
        ...
        const beforeUpdateSnapshot = offDay.toObject();
        if (offDate) {
        ...
        if (reason)
            offDay.reason = reason;
        if (mealTypes)
            offDay.mealTypes = mealTypes;
        if (billingDeduction !== undefined)
            offDay.billingDeduction = billingDeduction;
        if (subscriptionExtension !== undefined)
            offDay.subscriptionExtension = subscriptionExtension;
        ...
        try {
            ...
            ['offDate', 'reason', 'mealTypes', 'subscriptionExtension', 'extensionDays'].forEach((k) => {
                if (JSON.stringify(beforeUpdateSnapshot[k]) !== JSON.stringify(after[k])) {
                    changes[k] = { before: beforeUpdateSnapshot[k], after: after[k] };
                }
            });
            if (Object.keys(changes).length > 0) {
                await MessOffDayAudit_1.default.create({ messId: messProfile._id, offDayId: offDay._id, action: 'update', by: messOwnerId, changes, snapshot: after });
            }
        }
        catch { }
        return res.json({
...
exports.updateMessOffDay = updateMessOffDay;
// Delete mess off day
const deleteMessOffDay = async (req, res, _next) => {
    try {
        const { id } = req.params;
        ...
        if (originalSubscriptionExtension) {
            try {
                ...
                for (const membership of memberships) {
                    ...
                    const entriesFromOffDay = [];
                    if (offDay.isRange && offDay.rangeStartDate && offDay.rangeEndDate) {
                        const s = new Date(offDay.rangeStartDate);
                        s.setHours(0, 0, 0, 0);
                        const e = new Date(offDay.rangeEndDate);
                        e.setHours(0, 0, 0, 0);
                        for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
                            const isFirst = d.getTime() === s.getTime();
                            const isLast = d.getTime() === e.getTime();
                            const meals = isFirst ? ((offDay.startDateMealTypes?.length ? offDay.startDateMealTypes : ['breakfast', 'lunch', 'dinner']))
                                : isLast ? ((offDay.endDateMealTypes?.length ? offDay.endDateMealTypes : ['breakfast', 'lunch', 'dinner']))
                                    : ['breakfast', 'lunch', 'dinner'];
                        ...
                        const copy = new Date(originalOffDate);
                        copy.setHours(0, 0, 0, 0);
                    ...
                    const leaveMealsForDate = (date) => {
                        ...
                        for (const lv of userLeaves) {
                            ...
                            const meals = isFirst ? (lv.startDateMealTypes?.length ? lv.startDateMealTypes : (lv.mealTypes?.length ? lv.mealTypes : ['breakfast', 'lunch', 'dinner']))
                                : isLast ? (lv.endDateMealTypes?.length ? lv.endDateMealTypes : (lv.mealTypes?.length ? lv.mealTypes : ['breakfast', 'lunch', 'dinner']))
                                    : (lv.mealTypes?.length ? lv.mealTypes : ['breakfast', 'lunch', 'dinner']);
                            meals.forEach((m) => out.add(m));
                        }
                        return out;
                    };
                    let missedMeals = 0;
                    for (const ent of entriesFromOffDay) {
                        const eligibleForPlan = ent.meals.filter((m) => plan.mealOptions?.[m]);
                        if (eligibleForPlan.length === 0)
                            continue;
                        const lvMeals = leaveMealsForDate(ent.date);
        ...
        try {
            await MessOffDayAudit_1.default.create({
                messId: messProfile._id,
                offDayId: offDay._id,
                action: 'delete',
                by: messOwnerId,
...
exports.deleteMessOffDay = deleteMessOffDay;
// Get mess off day statistics
const getMessOffDayStats = async (req, res, _next) => {
    try {
        const messOwnerId = req.user.id;
        // Get mess ID from mess owner
        const messProfile = await getMessProfileForOwner(messOwnerId);
        ...
        const totalOffDays = await MessOffDay_1.default.countDocuments({
            messId: messId
        });
        const thisWeekStats = await MessOffDay_1.default.countDocuments({
            messId: messId,
            createdAt: { $gte: startOfWeek }
        });
        const thisMonthStats = await MessOffDay_1.default.countDocuments({
            messId: messId,
            createdAt: { $gte: startOfMonth }
        });
        const upcomingOffDays = await MessOffDay_1.default.countDocuments({
            messId: messId,
...
exports.getMessOffDayStats = getMessOffDayStats;
// Get default off day settings
const getDefaultOffDaySettings = async (req, res, _next) => {
    try {
        const messOwnerId = req.user.id;
        // Get mess ID from mess owner
        const messProfile = await getMessProfileForOwner(messOwnerId);
        ...
        const settings = await DefaultOffDaySettings_1.default.findOne({ messId: messProfile._id });
        return res.json({
            success: true,
            data: {
                settings
...
exports.getDefaultOffDaySettings = getDefaultOffDaySettings;
// Save default off day settings
const saveDefaultOffDaySettings = async (req, res, _next) => {
    try {
        const messOwnerId = req.user.id;
        ...
        // Upsert settings
        const settings = await DefaultOffDaySettings_1.default.findOneAndUpdate({ messId: messProfile._id }, {
            messId: messProfile._id,
            pattern,
            weeklySettings,
            monthlySettings,
        ...
        try {
            ...
            if (room) {
                ...
                const lines = ['📢 **Default Mess Off Schedule Updated**', ''];
                ...
                if (monthlySettings?.enabled) {
                    const days = (monthlySettings.daysOfMonth || []).sort((a, b) => a - b).join(', ');
...
exports.saveDefaultOffDaySettings = saveDefaultOffDaySettings;
//# sourceMappingURL=messOffDayController.js.map