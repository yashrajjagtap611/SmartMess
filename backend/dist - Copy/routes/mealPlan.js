...
const ensureMealOptionsCompatibility = async (mealPlans) => {
    for (const mealPlan of mealPlans) {
        if (!mealPlan.mealOptions) {
            ...
            await MealPlan_1.default.findByIdAndUpdate(mealPlan._id, { mealOptions: defaultMealOptions });
...
router.get('/', requireAuth_1.default, async (req, res, _next) => {
    try {
        ...
        const mealPlans = await MealPlan_1.default.find({ messId: messProfile._id.toString() }).sort({ createdAt: -1 });
...
router.get('/:id', requireAuth_1.default, async (req, res, _next) => {
    ...
    try {
        ...
        const mealPlan = await MealPlan_1.default.findOne({ _id: mealPlanId, messId: messProfile._id.toString() });
        if (!mealPlan) {
        ...
        if (!mealPlan.mealOptions) {
            ...
            // Update the document to include mealOptions
            await MealPlan_1.default.findByIdAndUpdate(mealPlanId, { mealOptions: mealPlan.mealOptions });
        }
...
router.put('/:id', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const mealPlanId = req.params['id'];
        ...
        const existingMealPlan = await MealPlan_1.default.findOne({ _id: mealPlanId, messId: messProfile._id.toString() });
        if (!existingMealPlan) {
            return res.status(404).json({ message: 'Meal plan not found' });
        }
        ...
        if (updateData.pricing !== undefined)
            updateFields.pricing = { amount: updateData.pricing.amount, period: updateData.pricing.period };
        ...
        if (updateData.leaveRules !== undefined)
            updateFields.leaveRules = { ...existingMealPlan.leaveRules, ...updateData.leaveRules };
        const updatedMealPlan = await MealPlan_1.default.findByIdAndUpdate(mealPlanId, updateFields, { new: true, runValidators: true });
...
router.delete('/:id', requireAuth_1.default, async (req, res, _next) => {
    ...
    try {
        ...
        await MealPlan_1.default.findByIdAndDelete(mealPlanId);
...
const validateMealPlan = (mealPlanData, isUpdate = false) => {
    const errors = [];
...
router.post('/migrate-meal-options', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const messProfile = await MessProfile_1.default.findOne({ userId: req.user.id });
        if (!messProfile) {
        ...
        const mealPlansToUpdate = await MealPlan_1.default.find({
            messId: messProfile._id.toString(),
            $or: [
...
router.get('/meal-plans', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
...
router.get('/meal-plans/:id', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const mealPlanId = req.params['id'];
        // First, get the mess profile to get the messId
        ...
        if (!mealPlan.mealOptions) {
            ...
            // Update the document to include mealOptions
            await MealPlan_1.default.findByIdAndUpdate(mealPlanId, { mealOptions: defaultMealOptions });
            // Update the local object for response
            mealPlan.mealOptions = defaultMealOptions;
        }
        return res.status(200).json({
...
router.post('/meal-plans', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const mealPlanData = req.body;
        // First, get the mess profile to get the messId
...
router.put('/meal-plans/:id', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const mealPlanId = req.params['id'];
        const updateData = req.body;
...
router.delete('/meal-plans/:id', requireAuth_1.default, async (req, res, _next) => {
    const userId = req.user.id;
    try {
        const mealPlanId = req.params['id'];
        // First, get the mess profile to get the messId
