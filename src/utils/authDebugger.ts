...
// This utility helps track and debug authentication-related issues
...
class AuthDebugger {
  ...
  private trackReloads(): void {
    ...
    const reloadCount = sessionStorage.getItem('authDebugReloadCount');
    this.reloadCount = reloadCount ? parseInt(reloadCount, 10) + 1 : 1;
    sessionStorage.setItem('authDebugReloadCount', this.reloadCount.toString());
  ...
  private trackNavigation(): void {
    ...
    history.pushState = function(...args) {
      ...
      AuthDebugger.getInstance().navigationHistory.push(`push: ${args[2]}`);
      return originalPushState.apply(this, args);
    };
    
    history.replaceState = function(...args) {
      console.log(`🧭 [AUTH DEBUG] Navigation - replaceState:`, args[2]);
      AuthDebugger.getInstance().navigationHistory.push(`replace: ${args[2]}`);
      return originalReplaceState.apply(this, args);
    };
    
    window.addEventListener('popstate', (event) => {
      console.log(`🧭 [AUTH DEBUG] Navigation - popstate:`, event);
      AuthDebugger.getInstance().navigationHistory.push(`popstate: ${window.location.pathname}`);
    });
  }
